...
<html>
    ...
    <body>
        ...
        <h2>Deletion</h2>
        <form method="GET" action="projectDelete.php"> <!--refresh page when submitted-->
	    ...
	    Table: <input type="text" name="delTable"> <br /><br />
	    Attribute: <input type="text" name="delAtt"> <br /><br />
            Value: <input type="text" name="delValue"> Note: use apostrophe (example: 'low-fat')<br /><br /> 
	    For cascade on delete situation, delete from Table: goals, with Attribute: name, with Value: 'decrease-sugar' (for an example, can choose any from goals) <br /><br />
	...
	function handleDeleteRequest() {
	    ...
	    //Getting the values from user and delete that row data from  the table
	     $Table = $_GET['delTable'];
	     $Attribute = $_GET['delAtt'];
             $Value = $_GET['delValue']; 
            //executeBoundSQL("DELETE FROM {$Table} WHERE {$Attribute} = {$Value}", $alltuples);
		executePlainSQL("DELETE FROM {$Table} WHERE {$Attribute} = {$Value}");
