...
<html>
    ...
    <body>
        ...
        <h2>Enter table name and value that you would like to insert</h2>
        <form method="POST" action="projectInsert.php"> <!--refresh page when submitted-->
		...
		contain (portions char(20), n_name char(30), i_name char(30), m_name char(30), r_name char(30), primary key (n_name, i_name, m_name, r_name))<br/><br/>
            ...
            Table: <input type="text" name="insTable"> Note: enter table name without apostrophe. <br /><br />
            Value: <input type="text" name="insValue"> Note: use apostrophes for the appropriate domain type and use commas to separate values if more than 1. <br /><br />
	...
	<form method="POST" action="project.php">
            <p><input type="submit" value="Go Back" name="reset"></p>
        </form>
	    ...
	    $value = $_POST['insValue'];
            executePlainSQL("insert into {$table} values ({$value})");
	    echo "<br>Values ({$value}) was successfully inserted into the following table: {$table} ";
            OCICommit($db_conn);	
